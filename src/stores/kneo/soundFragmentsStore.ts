...
export const useSoundFragmentStore = defineStore('soundFragmentStore', () => {
    ...
    const fetchSoundFragments = async (page = 1, pageSize = 10) => {
        const response = await apiClient.get(`/api/thomas-lee/soundfragments?page=${page}&size=${pageSize}`, {});
    ...
    const fetchSoundFragment = async (id: string) => {
        const response = await apiClient.get(`/api/thomas-lee/soundfragments/${id}`);
        if (response?.data?.payload) {
    ...
    const save = async (data: SoundFragmentSave, id?: string) => {
        const response = await apiClient.post(`/api/thomas-lee/soundfragments/${id}`, data);
        if (response?.data) {
