...
import EmailVerifyFields from './EmailVerifyFields.vue'
...
onMounted(async () => {
  try {
    if (props.brandSlug) {
      const station = await submissionStore.getStation(props.brandSlug)
      form.value.brand = station?.name || props.brandSlug
    ...
    form.value.brand = props.brandSlug || ''
    policy.value = ''
  }
...
async function handleSubmit() {
  ...
  try {
    ...
    await submissionStore.submit(props.brandSlug, payload)
