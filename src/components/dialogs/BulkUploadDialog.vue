<template>
  <n-modal v-model:show="showDialog" preset="dialog" title="Bulk Upload Sound Fragments" style="width: 700px;">
    <n-space vertical>
      <n-upload ref="uploadRef" :multiple="true" :show-file-list="false" v-model:file-list="fileList"
        :accept="'.mp3,.wav,.flac,.ogg,.m4a,.aac'" :custom-request="handleFileUpload" :disabled="isUploading">
        <n-button :disabled="isUploading">Choose Files</n-button>
      </n-upload>

      <n-data-table :columns="columns" :data="tableData" :bordered="false" :pagination="false" size="small" />
    </n-space>
    ...
    <template #action>
      <n-space>
        ...
        <n-button v-if=" !uploadCompleted " type="primary" @click="handleUpload"
          :disabled="fileList.length === 0 || isUploading" :loading="isUploading">
          Upload
        </n-button>
      </n-space>
    </template>
      ...
      },
      {
        ...
        render( row: any ) {
          ...
          const needsBlink =
            ...
            st.status === 'metadata_extracted' ||
            st.status === 'creating_entity'

          return h(
            NText,
            {
              style: 'font-size:12px;color:#999;',
              class: needsBlink ? 'status-blink' : ''
            },
