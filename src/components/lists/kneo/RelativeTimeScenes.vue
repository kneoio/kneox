<template>
  <n-grid :cols="isMobile ? 1 : 6" x-gap="12" y-gap="12" class="p-4">
    <n-gi>
      <n-page-header>
        <template #title>Relative Time Scenes</template>
        <template #footer>
...
export default defineComponent({
  ...
  setup() {
    ...
    const STORAGE_KEY = 'relativeTimeScenes.list.filters';
    const STORAGE_SHOW_KEY = 'relativeTimeScenes.list.showFilters';
    ...
    const rows = computed<any[]>(() => {
      ...
      return entries.map((sc: ScriptScene) => {
        ...
        return {
          ...
          timingMode: (sc as any).timingMode || SceneTimingMode.RELATIVE_TO_STREAM_START,
    ...
    const handleNewClick = () => {
      router.push({ name: 'RelativeTimeSceneForm', params: { id: 'new' } });
    };
    ...
    const getRowProps = (row: any) => {
      ...
      return {
        ...
        onClick: (e: MouseEvent) => {
          ...
          router.push({ name: 'RelativeTimeSceneForm', params: { id: row.id } });
        }
      };
    };
