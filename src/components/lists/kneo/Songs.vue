<template>
  ...
  <n-modal v-model:show="showBrandUpdateDialog" preset="dialog" title="Bulk Brand Update" :style="{ backgroundColor: dialogBackgroundColor }">
    <n-space vertical>
      ...
      <n-text depth="3">
        This will update {{ checkedRowKeys.length }} song(s).
      </n-text>
    </n-space>
...
export default defineComponent( {
  ...
  setup( props ) {
    ...
    const STORAGE_KEY = 'songs.list.filters';
    const STORAGE_SHOW_KEY = 'songs.list.showFilters';
    ...
    const confirmBrandUpdate = async () => {
      ...
      try {
        ...
        message.success(`Updated ${result.updatedCount} song(s) successfully`);
    ...
    const getRowProps = ( row: SoundFragment ) => {
      return {
        ...
        onClick: ( e: MouseEvent ) => {
          ...
          if ( props.brandName ) {
            router.push( { name: 'EditSong', params: { brandName: props.brandName, id: row.id } } );
          } else {
    ...
    const handleNewClick = () => {
      if ( props.brandName ) {
        router.push( { name: 'EditSong', params: { brandName: props.brandName, id: 'new' } } );
      } else {
        router.push( '/outline/songs/new' );
      }
    };
